...
export function EwsDetailModal({
  ...
  const getDefaultType = () => {
    if (isKepsek) return "SUPERVISI_KEPSEK"
    if (isWaliKelas) return "KONFIRMASI_WALI"
    return "KONSELING_INDIVIDU"
  }

  const [interventionType, setInterventionType] = React.useState(getDefaultType())
  ...
  React.useEffect(() => {
    if (notification) {
      if (isKepsek) setInterventionType("SUPERVISI_KEPSEK")
      else if (isWaliKelas) setInterventionType("KONFIRMASI_WALI")
      else setInterventionType("KONSELING_INDIVIDU")

      setActionNotes("")
      setStudentProgress("DALAM_PEMANTAUAN")
    }
  }, [notification, userRole, isKepsek, isWaliKelas])
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-900/50 backdrop-blur-xs animate-in fade-in-0 duration-150">
      <div className="relative w-full max-w-3xl max-h-[90vh] flex flex-col rounded-3xl bg-white border border-slate-200 shadow-2xl overflow-hidden">
        
        {/* Modal Header */}
        <div className="p-5 sm:p-6 bg-slate-50/70 border-b border-slate-200 flex items-start justify-between gap-4 shrink-0">
          <div className="flex items-start gap-3 sm:gap-4">
            <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-slate-700 font-extrabold text-base shrink-0 shadow-xs">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-lg text-[11px] font-semibold bg-white text-slate-600 border border-slate-200">
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-9 h-9 rounded-xl bg-white hover:bg-slate-100 border border-slate-200 text-slate-500 hover:text-slate-800 flex items-center justify-center cursor-pointer shrink-0 transition-all active:scale-95 shadow-xs"
          >
        ...
        {/* Tab Navigation */}
        <div className="px-5 sm:px-6 pt-3 bg-slate-50/40 border-b border-slate-200 flex items-center gap-2 overflow-x-auto shrink-0">
          <button
            type="button"
          ...
          >
            ...
            <span>Pemicu ({notification.risk_factors.length})</span>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <span>{isKepsek ? "Supervisi & Disposisi" : isWaliKelas ? "Bimbingan Kelas" : "Tindak Lanjut BK"}</span>
          </button>
        </div>
        ...
        <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-4 bg-white">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500">
                        Target Komunikasi: {isKepsek ? "Supervisi Pimpinan & Evaluasi Kasus" : isWaliKelas ? "Pendampingan Santai Wali Kelas" : "Layanan Bimbingan Konseling Sekolah"}
                      </p>
                    </div>
                  </div>
          ...
          {activeTab === "interventions" && (
            <div className="space-y-5 animate-in fade-in-0 duration-150">
              ...
              <form onSubmit={handleSubmitIntervention} className="p-5 rounded-2xl bg-slate-50/70 border border-slate-200/80 space-y-4">
                <div className="flex items-center gap-2 border-b border-slate-200/60 pb-2.5">
                  <IconHandshake className="w-4 h-4 text-indigo-600" />
                  <h4 className="text-xs sm:text-sm font-bold text-slate-900">
                    {isKepsek
                      ? "Disposisi & Arahan Supervisi Kepala Sekolah"
                      : isWaliKelas
                      ? "Catat Pendampingan / Rujukan Wali Kelas"
                      : "Catat Sesi Konseling & Intervensi BK"}
                  </h4>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      className="w-full h-10 px-3 rounded-xl bg-white border border-slate-200 text-xs font-semibold text-slate-800 focus:border-blue-500 focus:outline-none shadow-xs"
                    >
                      {isKepsek ? (
                        <>
                          <option value="SUPERVISI_KEPSEK">Arahan Supervisi Kepala Sekolah</option>
                          <option value="DISPOSISI_KEPSEK">Disposisi Penanganan ke Guru BK &amp; Wali Kelas</option>
                          <option value="PEMANGGILAN">Instruksi Pemanggilan Orang Tua Bersama</option>
                          <option value="LAINNYA">Catatan Supervisi Khusus</option>
                        </>
                      ) : isWaliKelas ? (
                        <>
                          ...
                          <option value="HOME_VISIT">Rujuk Kasus ke Guru BK Sekolah</option>
                          <option value="LAINNYA">Tindakan Wali Kelas Lainnya</option>
                        </>
                      ) : (
                        <>
                          ...
                          <option value="LAINNYA">Tindakan Layanan Lainnya</option>
                        </>
                      )}
                    </select>
                  </div>
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-bold text-slate-700">
                    {isKepsek
                      ? "Catatan Arahan Supervisi / Disposisi"
                      : isWaliKelas
                      ? "Catatan Hasil Pendampingan Kelas"
                      : "Catatan Hasil Konseling / Tindakan BK"}
                  </Label>
                  <Textarea
                    value={actionNotes}
                    ...
                    placeholder={
                      isKepsek
                        ? "Contoh: Mohon Guru BK berkoordinasi dengan Wali Kelas untuk pemanggilan orang tua minggu ini guna evaluasi tugas Moodle..."
                        : isWaliKelas
                        ? "Contoh: Mengonfirmasi alasan siswa inaktif di LMS. Siswa menyampaikan kendala gawai dan berjanji menyusul tugas..."
                        : "Contoh: Siswa dipanggil ke ruang BK untuk sesi bimbingan pribadi mengenai komitmen belajar dan penyelesaian tugas..."
                    }
                    rows={3}
                    className="text-xs resize-none bg-white focus:bg-white border-slate-200 shadow-xs"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
                  ...
                  <div className="flex justify-end pt-5 sm:pt-0">
                    ...
                    >
                      ...
                      <span>
                        {isKepsek
                          ? "Simpan Disposisi Supervisi"
                          : isWaliKelas
                          ? "Simpan Pendampingan Kelas"
                          : "Simpan Tindak Lanjut BK"}
                      </span>
                    </Button>
                  </div>
                </div>
